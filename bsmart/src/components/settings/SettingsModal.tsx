  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      ...
      >
        ...
        {/*
          外層：overflow-hidden + py-8 — 讓內層滾動容器上下各內縮 32px，
                避免 scrollbar 觸及卡片 28px 圓角。
          內層：h-full overflow-y-auto — 真正的滾動容器，scrollbar 繪製於此。
        */}
        <div className="flex-1 min-w-0 overflow-hidden py-8 relative">
          <div className="h-full overflow-y-auto px-8 styled-scrollbar">
            {tab === "general" && (
        ...
        <button
          type="button"
          onClick={onClose}
          className="absolute right-6 top-6 rounded-full p-2 text-xl text-[#999] hover:bg-[#F0F0F0] hover:text-[#555] transition-colors z-10"
          title="關閉"
        >
          ✕
        </button>
